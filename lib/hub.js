...
function create() {
  ...
  function eachMatchingListener(event, re, fn) {
    var evt;
  ...
  function eachMatchingMatcher(event, re, fn) {
  ...
  function findListeners(type, event, re) {
    if (re) {
      return cachedEntries(listenersCache, type, event, re,
          eachMatchingListener);
    }
    var entry = listeners[event];
  ...
  function findMatchers(type, event, re) {
    return cachedEntries(matchersCache, type, event, re, eachMatchingMatcher);
  }
  ...
  return {
    ...
    listenersMatching: function (event) {
      ...
      eachMatchingListener(event, re, function (entry) {
        arrayPush.apply(matches, entryListeners(entry));
      });
      eachMatchingMatcher(event, re, function (entry) {
        arrayPush.apply(matches, entryListeners(entry));
      });
      return matches;
    },
