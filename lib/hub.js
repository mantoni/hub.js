...
function emitAndMerge(listeners, scope, args, returnValues, callback) {
  if (listeners) {
    emit(listeners, scope, args, returnValues, function (err) {
      if (err) {
...
Hub.prototype.emit = function (event) {
  ...
  if (matchers) {
    ...
    var onReturn = function (err) {
      var result = strategyFn(returnValues);
      if (beforeReturn) {
    ...
    emit(matchers, scope, args, returnValues, function (err) {
      if (err) {
        handleError(err, callback);
      } else if (!stopped) {
        emitAndMerge(listeners, null, args, returnValues, onReturn);
      }
    });

  } else {

    emitAndMerge(listeners, null, args, returnValues, function (err, result) {
      if (callback) {
        callback(err, strategyFn(returnValues));
      } else if (err) {
        throw err;
      }
    });
