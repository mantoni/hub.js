...
View.prototype = {
  ...
  removeAllListeners: function (event) {
    if (arguments.length) {
      ...
      this.hub.removeAllMatching(this.namespace + '.**');
    }
  }
};
  ...
  return {
    ...
    emit: function (event) {
      ...
      function after(err, values) {
        var res   = result(scope, values);
        var args  = [err, res];
        var empty = [];
        emitPhase(findListeners, 'after', args, empty, function () {
          emitPhase(findMatchers, 'after', args, empty, function () {
            done(scope, callback, err, res);
          });
        });
      }
