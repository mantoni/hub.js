...
function buildChain(cache, fn) {
  return function (scope, re, callback) {
    run(cache.iterator(), fn)(scope, re, callback);
  };
}
...
function emitInternal(scope, callback) {
  ...
  if (!filters.isEmpty() || !listeners.isEmpty()) {
    run(filters.iterator(), function (scope, re, callback) {
...
function getter(list, wList) {
  return function (event) {
    if (event.indexOf('*') === -1) {
      return list[event] || store();
    }
    var i = matcherIndex(wList, event);
    if (i !== -1) {
      return wList[i].store;
    }
    return store();
  };
}
...
function hub() {
  ...
  return {
    ...
    emit: function (event) {
      ...
      var meta = filtersMatching(scope.event);
      meta.chain(scope, meta.re, function (err, values) {
