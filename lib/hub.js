...
function hub() {
  ...
  var addListener = function (event, fn) {
    this.emit('newListener', event, fn, function () {
      ...
      if (event.indexOf('*') === -1) {
        ...
        matcher(wildcardListeners, event).on.push(fn);
      }
    });
  };
  ...
  return {
    ...
    addFilter: function (event, fn) {
      this.emit('newFilter', event, fn, function () {
        var re = eventRE(event, true);
        clearMatchingKeys(filtersCache, re);
        if (event.indexOf('*') === -1) {
          var arr = filters[event];
          if (arr) {
            arr.push(fn);
          } else {
            filters[event] = [fn];
          ...
          matcher(wildcardFilters, event).on.push(fn);
        }
      });
    },

    removeFilter: function (event, fn) {
      this.emit('removeFilter', event, fn, function () {
        var entry, re = eventRE(event, true);
        clearMatchingKeys(filtersCache, re);
        if (event.indexOf('*') === -1) {
          entry = filters[event];
        } else {
          var i = matcherIndex(wildcardFilters, event);
          if (i !== -1) {
            entry = wildcardFilters[i].on;
          }
        }
        if (entry) {
          remove(entry, fn);
