...
function emit(listeners, scope, values, callback) {
  if (listeners) {
...
function viewDelegateTwoArgs(method) {
  viewProto[method] = function (event, callback) {
...
function hub() {
  ...
  var once = function (event, fn) {
    ...
    if (fn.length) {
      ...
      eval('proxy = (function (' + vars.substring(0, fn.length * 2 - 1) +
        ') { hub.un(event, proxy); return fn.apply(this, arguments); });');
    } else {
      proxy = function () {
        hub.un(event, proxy);
  ...
  var removeListener = function (event, fn) {
  ...
  var addFilter = function (event, fn) {
    listener(event).f.push(fn);
  };

  var removeFilter = function (event, fn) {
  ...
  return {
    ...
    emit: function (type) {
      ...
      var filters = findListeners('f', event, re);
