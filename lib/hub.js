...
function cachedEntries(cache, type, event, re, each) {
  if (!cache.hasOwnProperty(event)) {
    ...
    each(event, re, function (entry) {
      cached = cached ? {
        f  : cached.f.concat(entry.f),
        on : cached.on.concat(entry.on)
      } : entry;
    });
    cache[event] = cached || {};
  }
...
function hub() {
  ...
  return {
    ...
    emit: function (event) {
      ...
      var matchers  = findMatchers('f', scope.event, re);
      var listeners = findListeners('f', scope.event, re);
      var fns = matchers && listeners ? matchers.concat(listeners) :
          (matchers || listeners);
